...
st.markdown("""
<style>
/* Police Times New Roman uniquement */
...
html, body, [data-testid="stAppViewContainer"], .main {
    background:
        radial-gradient(circle at top right, rgba(59,130,246,.15), transparent 30%),
        radial-gradient(circle at bottom left, rgba(0,200,150,.07), transparent 30%),
        #050b16 !important;
...
/* ── KPI Cards ── */
.kpi-card {
    background: linear-gradient(145deg, rgba(15,23,42,.97), rgba(17,24,39,.97));
    border-radius: 16px;
...
.kpi-value { font-size:36px; font-weight:800; color:var(--accent,#3b82f6); line-height:1.1; letter-spacing:-1px; }
...
/* ── Page Header ── */
.page-header {
    background: linear-gradient(135deg, rgba(17,24,39,.97), rgba(30,58,138,.88));
...
.page-header h1 {
    ...
    letter-spacing:-.5px; margin:0 !important; color:#f0f6ff !important;
}
.page-header p { margin:4px 0 0 !important; opacity:.6; font-size:12px !important; }
.header-badge {
...
.status-online { background:#22c55e; box-shadow:0 0 8px #22c55e; animation: pulse 2s infinite; }
@keyframes pulse { 0%,100%{opacity:1;} 50%{opacity:.5;} }

/* ── Alert Cards ── */
.alert-card-red    { background:rgba(220,38,38,.1);  border:1px solid rgba(220,38,38,.3);  border-left:3px solid #dc2626; border-radius:10px; padding:11px 13px; margin-bottom:7px; transition:background .2s; }
.alert-card-orange { background:rgba(234,88,12,.1);  border:1px solid rgba(234,88,12,.3);  border-left:3px solid #ea580c; border-radius:10px; padding:11px 13px; margin-bottom:7px; transition:background .2s; }
.alert-card-green  { background:rgba(22,163,74,.1);  border:1px solid rgba(22,163,74,.3);  border-left:3px solid #16a34a; border-radius:10px; padding:11px 13px; margin-bottom:7px; transition:background .2s; }
.alert-card-red:hover    { background:rgba(220,38,38,.16); }
.alert-card-orange:hover { background:rgba(234,88,12,.16); }
.alert-card-green:hover  { background:rgba(22,163,74,.16); }
.alert-machine { font-weight:700; font-size:13px; color:#e2e8f0; }
...
.badge-orange  { background:rgba(234,88,12,.25); color:#fed7aa; }
.badge-green   { background:rgba(22,163,74,.25); color:#86efac; }

/* ── Section Titles ── */
.section-title {
    font-size:14px; font-weight:700; color:#e2e8f0;
    display:flex; align-items:center; gap:8px;
    padding-bottom:10px; border-bottom:1px solid #21262d; margin-bottom:14px;
}
.section-title .dot { width:7px; height:7px; border-radius:50%; background:#3b82f6; display:inline-block; flex-shrink:0; }
...
.tab-btn.active { background:rgba(59,130,246,.18); border-color:#3b82f6; color:#93c5fd; }
...
.perf-table th {
    ...
    padding:10px 12px; background:rgba(13,17,23,.8);
    border-bottom:1px solid #21262d; text-align:left;
}
.perf-table td { padding:10px 12px; font-size:12px; color:#c9d1d9; border-bottom:1px solid rgba(33,38,45,.6); }
...
.toast-bar {
    ...
    font-size:12px; color:#86efac; display:flex; align-items:center; gap:8px;
}

/* ── Sidebar ── */
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #080e1a 0%, #0d1117 100%) !important;
...
body.theme-light .perf-table th { background: #f8fafc !important; color: #64748b !important; border-color: #e2e8f0 !important; }
body.theme-light .perf-table td { color: #1e293b !important; border-color: #f1f5f9 !important; }
...
.shift-tab:hover { border-color:#3b82f6; color:#93c5fd; }
.shift-tab.active { background:rgba(59,130,246,.18); border-color:#3b82f6; color:#93c5fd; }
...
/* ── Progress bar ── */
.progress-wrap { background:rgba(255,255,255,.06); border-radius:4px; height:6px; margin-top:6px; overflow:hidden; }
...
def sidebar_navigation():
    with st.sidebar:
        ...
        <div style="text-align:center;padding:14px 10px;
            ...
            <div style="font-size:14px;font-weight:700;color:#e2e8f0;">{st.session_state.nom_user}</div>
        ...
        st.markdown("---")
        st.markdown("<div style='font-size:10px;color:#484f58;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:0 4px 6px;'>Navigation</div>", unsafe_allow_html=True)
        ...
        # ── Theme switcher ──
        st.markdown("<div style='font-size:10px;color:#484f58;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:0 4px 6px;'>Apparence</div>", unsafe_allow_html=True)
        theme = st.radio("", ["🌙 Dark", "☀️ Light"], horizontal=True,
        ...
        # Simulation toggle
        st.markdown("<div style='font-size:10px;color:#484f58;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:0 4px 6px;'>Mode données</div>", unsafe_allow_html=True)
        ...
        st.markdown("---")
        st.markdown(f"""
        <div style="font-family:monospace;font-size:11px;color:#484f58;text-align:center;
...
PLOT_LAYOUT_BASE = dict(
    ...
    legend=dict(orientation="h", yanchor="bottom", y=1.02, font=dict(size=11, family='Times New Roman, Times, serif')),
    xaxis=dict(gridcolor='rgba(255,255,255,0.05)', tickfont=dict(size=11, family='Times New Roman, Times, serif')),
)
...
def pl(yaxis_title=None, yaxis_range=None, height=None, **extra):
    ...
    yax = dict(gridcolor='rgba(255,255,255,0.05)', tickfont=dict(size=11, family='Times New Roman, Times, serif'))
    if yaxis_title: yax['title'] = yaxis_title
...
def page_accueil(df, df_tps):
    ...
    kpis = [
        ...
        (f"{total_inter:.0f}",            "Total Interruptions",    "minutes cumulées",        "#f59e0b",  "MIN"),
        (f"{total_markers}",              "Total Markers",          f"{jours} jours analysés", "#a855f7",  "PCS"),
    ]
    cols = st.columns(5)
    ...
    with col1:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            delta={'reference':75,'increasing':{'color':"#22c55e"},'decreasing':{'color':"#ef4444"},'font':{'size':14,'family':'Times New Roman, Times, serif'}},
            gauge={
                'axis':{'range':[0,100],'tickwidth':1,'tickcolor':'#484f58','tickfont':{'color':'#8b949e','size':10,'family':'Times New Roman, Times, serif'}},
                ...
                'threshold':{'line':{'color':"#f59e0b",'width':2},'thickness':.75,'value':75}
            },
        ...
        fig_gauge.update_layout(height=280, margin=dict(t=40,b=10,l=20,r=20),
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color='#c9d1d9', font_family='Times New Roman, Times, serif')
    ...
    with col2:
        ...
        alert_col(c1, critiques,   "alert-card-red",    "badge-red",    "CRITIQUE",  "#fca5a5", "🔴 Critique",       "Aucune machine")
        alert_col(c2, attention,   "alert-card-orange", "badge-orange", "ATTENTION", "#fed7aa", "🟠 À surveiller",   "Aucune machine")
        alert_col(c3, ok_machines, "alert-card-green",  "badge-green",  "OK",        "#86efac", "🟢 Objectif atteint","Aucune machine")
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;">
        <span class="mini-stat">⚠️ Interruptions : <strong style="color:#ef4444;">{total_inter:.0f} min</strong></span>
        <span class="mini-stat">📊 CODA : <strong style="color:#a855f7;">{total_coda:.0f} min</strong></span>
        <span class="mini-stat">🔄 ΔT Matelas : <strong style="color:#64748b;">{total_dt_mat:.0f} min</strong></span>
        <span class="mini-stat">📋 Markers : <strong style="color:#93c5fd;">{total_markers}</strong></span>
        <span class="mini-stat">📅 Jours : <strong style="color:#86efac;">{jours}</strong></span>
    </div>""", unsafe_allow_html=True)
    ...
    for _, r in df_tps.iterrows():
        tps = r['TPS (%)']
        if tps >= 75:   pill = "background:rgba(34,197,94,.18);color:#86efac;";  statut = "✅ OK"
        elif tps >= 40: pill = "background:rgba(245,158,11,.18);color:#fde68a;"; statut = "⚠️ NOK"
        else:           pill = "background:rgba(239,68,68,.18);color:#fca5a5;";  statut = "🔴 CRITIQUE"
        ecart = r['Écart (%)']
        ecart_html = f'<span style="color:#22c55e;">+{ecart:.1f}%</span>' if ecart >= 0 else f'<span style="color:#ef4444;">{ecart:.1f}%</span>'
        rows_html += f"""
        <tr>
            <td style="font-weight:700;color:#e2e8f0;">{r['Machine']}</td>
            <td><span class="tps-pill" style="{pill}">{tps:.1f}%</span></td>
            <td style="color:#8b949e;">75%</td>
            <td>{ecart_html}</td>
            <td style="color:#93c5fd;">{r['ADV (%)']:.1f}%</td>
...
def page_tps(df, df_tps):
    ...
    st.markdown('<div class="section-title"><span class="dot"></span> TPS par Machine vs Objectif (75%)</div>', unsafe_allow_html=True)
    colors = ['#ef4444' if t < 40 else '#f59e0b' if t < 75 else '#22c55e' for t in df_tps['TPS (%)']]
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df_tps['Machine'], y=df_tps['TPS (%)'], marker_color=colors, name='TPS Réel',
        text=[f"{v:.1f}%" for v in df_tps['TPS (%)']], textposition='outside', textfont=dict(color='#c9d1d9',size=11, family='Times New Roman, Times, serif')))
    fig.add_trace(go.Scatter(x=df_tps['Machine'], y=[75]*len(df_tps), mode='lines', name='Objectif 75%',
        line=dict(color='#f59e0b', width=2, dash='dash')))
    fig.update_layout(**pl(yaxis_title="TPS (%)", yaxis_range=[0,110], height=380))
    st.plotly_chart(fig, use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        st.markdown('<div class="section-title"><span class="dot"></span> Heatmap Cutting Time</div>', unsafe_allow_html=True)
        if 'DATE' in df.columns:
            pivot = df.pivot_table(values='CUTTING TIME', index='Machine', columns=df['DATE'].dt.strftime('%d/%m'), aggfunc='sum', fill_value=0)
    ...
    with col2:
        st.markdown('<div class="section-title"><span class="dot"></span> Évolution TPS journalier</div>', unsafe_allow_html=True)
        if 'DATE' in df.columns:
            tps_jour = [{'Date': d, 'TPS Moyen (%)': calculer_tps_adv(df[df['DATE']==d])['TPS (%)'].mean()}
                        for d in sorted(df['DATE'].unique())]
            df_tj = pd.DataFrame(tps_jour)
            fig_evol = go.Figure()
            fig_evol.add_trace(go.Scatter(x=df_tj['Date'], y=df_tj['TPS Moyen (%)'],
                mode='lines+markers+text', text=[f"{v:.1f}%" for v in df_tj['TPS Moyen (%)']],
                textposition='top center', line=dict(color='#3b82f6',width=2), marker=dict(size=7)))
            fig_evol.add_trace(go.Scatter(x=df_tj['Date'], y=[75]*len(df_tj), mode='lines',
                name='Objectif', line=dict(color='#ef4444',dash='dash',width=1.5)))
            fig_evol.update_layout(**pl(yaxis_title="TPS (%)", yaxis_range=[0,100], height=320))
            st.plotly_chart(fig_evol, use_container_width=True)

    st.markdown("---")
    st.markdown('<div class="section-title"><span class="dot"></span> Jauges TPS par Machine</div>', unsafe_allow_html=True)
    n_cols = min(3, len(df_tps))
    cols = st.columns(n_cols)
    for i, (_, row) in enumerate(df_tps.iterrows()):
        with cols[i % n_cols]:
            color = "#22c55e" if row['TPS (%)'] >= 75 else "#f59e0b" if row['TPS (%)'] >= 40 else "#ef4444"
            fig_g = go.Figure(go.Indicator(
                ...
                gauge={'axis':{'range':[0,100]},'bar':{'color':color},
                    ...
                    'threshold':{'line':{'color':"#f59e0b",'width':2},'thickness':.75,'value':75}},
                title={'text':row['Machine'],'font':{'color':'#c9d1d9','family':'Times New Roman, Times, serif'}}
            ))
...
def page_pertes(df, df_tps):
    ...
    for col, val, lbl, color in [
        (kpi_cols[0], f"{t_int:.0f}", "Interruptions (min)", "#ef4444"),
        (kpi_cols[1], f"{t_coda:.0f}", "CODA (min)",          "#a855f7"),
        (kpi_cols[2], f"{t_pos:.0f}",  "DT_POSIT (min)",      "#f59e0b"),
        (kpi_cols[3], f"{t_pm:.0f}",   "POSIT/Marker",        "#06b6d4"),
        (kpi_cols[4], f"{t_mat:.0f}",  "ΔT_Matelas (min)",    "#64748b"),
    ]:
        with col:
    ...
    with col1:
        ...
        for name, col_name, color in [
            ('Interruptions','Interruptions (min)','#ef4444'),
            ('CODA','CODA (min)','#a855f7'),
            ('DT_POSIT','DT_POSIT (min)','#f59e0b'),
            ('POSIT/Marker','POSIT/Marker','#06b6d4'),
            ('ΔT_Matelas','ΔT_Matelas (min)','#64748b'),
        ]:
    ...
    with col2:
        ...
        fig_pie = px.pie(values=[t_int,t_coda,t_pos,t_pm,t_mat],
            ...
            color_discrete_sequence=['#ef4444','#a855f7','#f59e0b','#06b6d4','#64748b'], hole=0.45)
    ...
    fig_p.add_trace(go.Bar(x=pareto['Source'],y=pareto['Total (min)'],
        marker_color=['#ef4444','#a855f7','#f59e0b','#06b6d4','#64748b'],
    ...
    fig_p.add_hline(y=80,line_dash="dash",line_color="#f59e0b",annotation_text="80%",yref='y2')
    ...
    for i, (_, row) in enumerate(df_tps.iterrows()):
        with cols_pie[i % n_cols]:
            fig_m = px.pie(values=[row['Interruptions (min)'],row['CODA (min)'],row['DT_POSIT (min)'],row['POSIT/Marker'],row['ΔT_Matelas (min)']],
                names=['Interruptions','CODA','DT_POSIT','POSIT/Marker','ΔT_Matelas'],
                color_discrete_sequence=['#ef4444','#a855f7','#f59e0b','#06b6d4','#64748b'],
            ...
            fig_m.update_layout(height=280,margin=dict(t=40,b=10,l=10,r=10),showlegend=False,
                paper_bgcolor='rgba(0,0,0,0)',font_color='#c9d1d9', font_family='Times New Roman, Times, serif')
...
def page_adv(df, df_tps):
    ...
    if 'DATE' not in df.columns or 'ADV' not in df.columns:
        st.warning("⚠️ Colonnes DATE ou ADV non disponibles.")
        return
    ...
    for col, val, lbl, color in [
        (kpi_cols[0], f"{adv_moy:.1f}%", "ADV Moyenne",       "#3b82f6"),
        (kpi_cols[1], f"{adv_min:.1f}%", "ADV Minimale",      "#ef4444"),
        (kpi_cols[2], f"{adv_max:.1f}%", "ADV Maximale",      "#22c55e"),
        (kpi_cols[3], f"{j_nok}j",       "Jours sous objectif","#f59e0b"),
    ]:
        with col:
            st.markdown(f"""<div class="kpi-card" style="--accent:{color};">
    ...
    colors_adv = ['#22c55e' if v >= 100 else '#ef4444' for v in df_adv['ADV (%)']]
    fig_adv = go.Figure()
    ...
    if not df_adv_m.empty:
        fig_adv_m = go.Figure(go.Bar(x=df_adv_m['Machine'],y=df_adv_m['ADV (%)'],
            marker_color=['#22c55e' if v >= 100 else '#ef4444' for v in df_adv_m['ADV (%)']],
            text=[f"{v:.1f}%" for v in df_adv_m['ADV (%)']],textposition='outside'))
        fig_adv_m.add_hline(y=100,line_dash="dash",line_color="#f59e0b",annotation_text="Objectif 100%")
...
def page_machine(df, df_tps):
    ...
    tps_v  = row['TPS (%)']
    color  = "#22c55e" if tps_v >= 75 else "#f59e0b" if tps_v >= 40 else "#ef4444"

    kpi_cols = st.columns(5)
    for col, val, lbl, c in [
        (kpi_cols[0], f"{tps_v:.1f}%",                "TPS",           color),
        (kpi_cols[1], f"{row['Interruptions (min)']:.0f} min","Interruptions","#ef4444"),
        (kpi_cols[2], f"{row['CODA (min)']:.0f} min",  "CODA",          "#a855f7"),
        (kpi_cols[3], f"{row['DT_POSIT (min)']:.0f} min","DT_POSIT",   "#f59e0b"),
        (kpi_cols[4], f"{row['ΔT_Matelas (min)']:.0f} min","ΔT_Matelas","#64748b"),
    ]:
        with col:
    ...
    with col1:
        fig_g = go.Figure(go.Indicator(
            ...
            gauge={'axis':{'range':[0,100]},'bar':{'color':color},
                'steps':[{'range':[0,40],'color':'rgba(239,68,68,.12)'},{'range':[40,75],'color':'rgba(245,158,11,.10)'},{'range':[75,100],'color':'rgba(34,197,94,.10)'}],
                'threshold':{'line':{'color':"#f59e0b",'width':2},'thickness':.75,'value':75}},
            title={'text':f"TPS {machine_sel} (%)","font":{'color':'#c9d1d9','family':'Times New Roman, Times, serif'}}
        ))
    ...
    with col2:
        fig_pie = px.pie(
            ...
            names=['Interruptions','CODA','DT_POSIT','POSIT/Marker','ΔT_Matelas'],
            color_discrete_sequence=['#ef4444','#a855f7','#f59e0b','#06b6d4','#64748b'], hole=0.42)
...
def page_donnees(df, df_tps):
    ...
    with col3:
        filtre_s = st.selectbox("⚙️ Statut", ['Tous'] + sorted(df['STATE'].dropna().unique().tolist())) if 'STATE' in df.columns else 'Tous'
...
def main():
    ...
    if 'DATE' in df.columns and not df.empty:
        st.sidebar.markdown("---")
        st.sidebar.markdown("<div style='font-size:10px;color:#484f58;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:0 4px 6px;'>Filtre par date</div>", unsafe_allow_html=True)
        ...
        if df_filtre.empty:
            st.warning(f"⚠️ Aucune donnée entre {date_debut.strftime('%d/%m/%Y')} et {date_fin.strftime('%d/%m/%Y')}")
            st.stop()
    ...
    st.markdown("""
    <div style='text-align:center;color:#484f58;font-size:11px;padding:20px 0 10px;letter-spacing:.5px;'>
